...
fn test_charset_convert_error_wraps_decode_and_encode_errors() {
    ...
    assert!(decode.to_string().contains("Failed to decode source charset"));
    ...
    assert!(encode.to_string().contains("Failed to encode target charset"));
}
...
fn test_charset_convert_error_maps_every_converter_error_domain() {
    ...
    let decode = CharsetDecodeError::new(Charset::UTF_8, CharsetDecodeErrorKind::malformed_unknown(), 2);
    let error = CharsetConvertError::from_transcode_error::<Utf8Codec, AsciiCodec>(
        Charset::UTF_8,
        Charset::ASCII,
