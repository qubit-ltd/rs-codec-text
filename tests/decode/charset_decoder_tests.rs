use qubit_codec::{TranscodeDecoder, TranscodeError};
use qubit_codec_text::{
    ...
    CharsetEncodeResult, Codec, MalformedAction, TranscodeStatus, Transcoder, Utf8Codec,
    Utf32U32Codec,
};
...
unsafe impl Codec for DecodeFlushErrorCodec {
    type Value = char;
    type Unit = u8;
    type DecodeError = CharsetDecodeError;
    type EncodeError = CharsetEncodeError;
...
#[test]
fn test_charset_decoder_finish_maps_decode_flush_errors() {
    let mut decoder = CharsetDecoder::new(DecodeFlushErrorCodec);
    let mut output = [];

    let error = decoder
        .finish(&mut output, 0)
        .expect_err("decode flush errors should be mapped through hooks");
