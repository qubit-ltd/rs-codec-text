...
fn test_ascii_codec_exposes_identity_and_limits() {
    ...
    assert_eq!(
        1,
        CharsetEncodeProbe::encode_len(&codec, 'A', 0).expect("ASCII is mappable")
    );
...
fn test_ascii_codec_encodes_ascii_and_reports_limits_and_unmappable_chars() {
    ...
    let error =
        CharsetEncodeProbe::encode_len(&codec, 'é', 0).expect_err("non-ASCII is unmappable");
    assert!(matches!(
        error.kind(),
...
fn test_ascii_codec_direct_function_items_cover_trait_methods() {
    ...
    let error: CharsetDecodeError = unsafe { decode(&mut codec, &[], 0) }.expect_err("closed tail");
    assert_eq!(
        CharsetDecodeErrorKind::IncompleteSequence {
            required: 1,
            available: 0,
    ...
    let error: CharsetEncodeError =
        unsafe { encode(&mut codec, &'é', &mut output, 0) }.expect_err("unmappable");
    assert_eq!(Some('é' as u32), error.value());
}
