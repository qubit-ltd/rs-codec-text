...
type EncodeFn = unsafe fn(&AsciiCodec, &char, &mut [u8], usize) -> CharsetEncodeResult<usize>;
...
fn test_ascii_codec_direct_function_items_cover_trait_methods() {
    ...
    let min_units: fn(&AsciiCodec) -> core::num::NonZeroUsize = <AsciiCodec as Codec>::min_units_per_value;
    let max_units: fn(&AsciiCodec) -> core::num::NonZeroUsize = <AsciiCodec as Codec>::max_units_per_value;
    ...
    let (decoded, consumed) = unsafe { decode(&codec, b"Z", 0) }.expect("decode ASCII");
    assert_eq!(('Z', 1), (decoded, consumed.get()));
    let error: CharsetDecodeError = unsafe { decode(&codec, &[], 0) }.expect_err("closed tail");
    assert_eq!(
        CharsetDecodeErrorKind::IncompleteSequence {
            required: 1,
            available: 0,
    ...
    let error: CharsetEncodeError = unsafe { encode(&codec, &'é', &mut output, 0) }.expect_err("unmappable");
    assert_eq!(Some('é' as u32), error.value());
}
