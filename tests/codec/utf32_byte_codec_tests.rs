...
type EncodeFn = unsafe fn(&mut Utf32ByteCodec, &char, &mut [u8], usize) -> CharsetEncodeResult<usize>;
...
fn test_utf32_byte_codec_exposes_encoder_and_decoder_contracts() {
    ...
    assert_eq!(
        ...
        <Utf32ByteCodec as Codec>::MAX_ENCODE_UNITS_PER_VALUE,
    );
...
fn test_utf32_byte_codec_encodes_and_decodes_bytes() {
    ...
    assert_eq!(4, unsafe {
        codec.encode(&'A', &mut output, 0).expect("encode UTF-32BE A")
    });
...
fn test_utf32_byte_codec_reports_closed_tail_and_invalid_units() {
    ...
    let error = unsafe { codec.decode(&[0x00, 0x00, 0x11, 0x00], 0) }.expect_err("non-scalar UTF-32 unit should fail");
...
fn test_utf32_byte_codec_direct_function_items_cover_trait_methods() {
    ...
    let inherent_charset: fn(Utf32ByteCodec) -> Charset = std::hint::black_box(Utf32ByteCodec::charset);
    let trait_charset: fn(&Utf32ByteCodec) -> Charset = std::hint::black_box(<Utf32ByteCodec as CharsetCodec>::charset);
