...
impl EncodeTranscodeEncodeErrorView for TranscodeEncodeError<CharsetEncodeError, char> {
    fn kind(&self) -> CharsetEncodeErrorKind {
...
impl EncodeTranscodeEncodeErrorSource for TranscodeEncodeError<CharsetEncodeError, char> {
    fn as_charset_error(&self) -> CharsetEncodeError {
        match self.clone() {
            ...
            TranscodeEncodeError::Unencodable { input_index, value } => {
                CharsetEncodeError::map_unencodable(Charset::ASCII, input_index, value)
            }
...
fn test_charset_encoder_is_transcode_encoder() {
    fn assert_transcode_encoder<T: TranscodeEncoder<Input = char, Output = u8>>() {}
...
impl_test_codec!(ReplacementAllUnencodableCodec, 1, |ch: char| ch.is_ascii() && ch != '?');
...
fn test_charset_encoder_transcoder_trait_methods_forward() {
    ...
    type TranscodeFn = fn(&mut Encoder, &[char], usize, &mut [u8], usize) -> EncoderResult<TranscodeProgress>;
    ...
    let transcode: TranscodeFn = std::hint::black_box(<Encoder as Transcoder>::transcode);
    let finish: OutputFn = std::hint::black_box(<Encoder as Transcoder>::finish);
    ...
    let progress =
        transcode(&mut encoder, &input, 0, &mut output, 0).expect("encoder should transcode through the trait");
...
fn test_charset_encode_policy_constructors_and_default_for() {
    let replace: fn(char) -> CharsetEncodePolicy = std::hint::black_box(CharsetEncodePolicy::replace);
    ...
    assert_eq!(CharsetEncodePolicy::DEFAULT_REPLACEMENT, ignore().replacement());
    assert_eq!(
    ...
    assert_eq!(CharsetEncodePolicy::DEFAULT_REPLACEMENT, report().replacement());
    assert_eq!(default(), CharsetEncodePolicy::default_for(&Utf8Codec).unwrap());
    assert_eq!(
    ...
    assert!(matches!(
        ...
        error
            if error.index() == input.len() + 1
                && error.kind() == CharsetEncodeErrorKind::InvalidInputIndex {
                    input_len: input.len(),
                }
    ));
    ...
    assert!(matches!(
        ...
        error
            if error.index() == beyond_output
                && error.kind() == CharsetEncodeErrorKind::InvalidOutputIndex {
