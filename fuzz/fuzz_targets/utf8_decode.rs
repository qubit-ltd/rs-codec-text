...
fuzz_target!(|data: &[u8]| {
    ...
    match core::str::from_utf8(data) {
        ...
        Err(error) if error.error_len().is_some() => {
            let expected = String::from_utf8_lossy(data).chars().collect::<Vec<_>>();
            let replace_written = replace_result.expect("complete malformed UTF-8 must be replaceable");
            assert_eq!(expected, replace_output[..replace_written]);
            assert!(report_result.is_err());
        }
        Err(_) => {
            let expected = String::from_utf8_lossy(data).chars().collect::<Vec<_>>();
            let replace_written = replace_result.expect("incomplete UTF-8 at EOF must be replaceable");
            assert_eq!(expected, replace_output[..replace_written]);
            assert!(report_result.is_err());
        }
    }
