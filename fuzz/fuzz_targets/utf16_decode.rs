...
fn fuzz_order(data: &[u8], byte_order: ByteOrder) {
    ...
    let mut report = CharsetDecoder::with_policy(Utf16ByteCodec::new(byte_order), CharsetDecodePolicy::report());
    ...
    if data.len().is_multiple_of(2) {
        ...
        let expected = decoded
            ...
            .collect::<Vec<_>>();
        let replace_written = replace_result.expect("complete UTF-16 input must be replaceable");
        assert_eq!(expected, replace_output[..replace_written]);
        if malformed {
            assert!(report_result.is_err());
        } else {
            let report_written = report_result.expect("well-formed UTF-16 must report success");
        ...
        let trailing_high_surrogate = units.last().is_some_and(|unit| (0xD800..=0xDBFF).contains(unit));
        ...
        let replace_written = replace_result.expect("odd UTF-16 byte tail at EOF must be replaceable");
        assert_eq!(expected, replace_output[..replace_written]);
        assert!(report_result.is_err());
    }
}
