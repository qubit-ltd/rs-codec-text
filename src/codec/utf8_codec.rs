...
/// Decodes the first UTF-8 character from a closed byte slice starting at `index`.
///
...
#[inline]
fn decode_prefix(input: &[u8], index: usize) -> CharsetDecodeResult<(char, core::num::NonZeroUsize)> {
    if index > input.len() {
