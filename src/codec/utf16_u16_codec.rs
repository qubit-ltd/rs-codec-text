...
///     codec.decode(&output[..written], 0).expect("valid UTF-16")
/// };
...
impl Codec for Utf16U16Codec {
    ...
    const MAX_ENCODE_UNITS_PER_VALUE: usize = Utf16::MAX_UNITS_PER_CHAR;

    const MAX_DECODE_UNITS_PER_VALUE: usize = Utf16::MAX_UNITS_PER_CHAR;
    ...
    #[inline]
    unsafe fn encode(&mut self, ch: &char, output: &mut [u16], output_index: usize) -> CharsetEncodeResult<usize> {
    ...
    if Utf16::is_high_surrogate(first) {
        ...
        match Utf16::compose_pair(first, second).and_then(Unicode::to_char) {
            ...
            None => {
                ...
                Err(CharsetDecodeError::new(Charset::UTF_16, kind, index.saturating_add(1))
...
fn encode_units_char(ch: char, output: &mut [u16], index: usize) -> usize {
    ...
    debug_assert!(
        SliceRange::range_fits(output.len(), index, length),
